...
import { certificationsText } from '../i18n/content';

export default function Certifications() {
  const { language, t } = useLanguage();
  const stats = [
    { number: '10+', label: t('certifications.stats.learningHours') },
    { number: '3', label: t('certifications.stats.keyCertifications') },
    { number: '2+', label: t('certifications.stats.learningPlatforms') },
    { number: '10+', label: t('certifications.stats.projectsCompleted') }
  ];
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-black text-zinc-950 dark:text-white tracking-tight">
            {t('certifications.headingMain')} <span className="text-teal-600 dark:text-teal-400">{t('certifications.headingHighlight')}</span>
          </h2>
        ...
        >
          {certificationsData.map((cert) => {
            const text = certificationsText[language][cert.id];
            return (
              <motion.div
                key={cert.id}
                ...
                <div>
                  ...
                  <div className={`relative h-40 bg-gradient-to-br ${cert.previewBg} p-5 flex flex-col justify-between border-b border-zinc-100 dark:border-zinc-800/80 overflow-hidden`}>
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                      <span className="text-[10px] font-black tracking-widest uppercase text-white/70">
                        {t('certifications.statementOfAccomplishment')}
                      </span>
                    ...
                    <div className="relative z-10 space-y-1">
                      <p className="text-[11px] text-white/80 font-medium">{t('certifications.awardedTo')}</p>
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed font-medium">
                      <span className="font-bold text-zinc-700 dark:text-zinc-300">{t('certifications.skillsLabel')} </span>
                      {text.skills}
                    </p>
                ...
                <div className="p-6 pt-0">
                  {cert.verifyUrl ? (
                    ...
                    <span className="w-full py-2.5 px-4 rounded-xl border border-zinc-200 dark:border-zinc-800 text-zinc-400 dark:text-zinc-600 text-xs font-bold flex items-center justify-center gap-1.5 cursor-not-allowed opacity-70">
                      ...
                      <span>{t('certifications.noPublicCredential')}</span>
                    </span>
                  )}
                </div>
              </motion.div>
